...
export default function () {
  ...
  let resLogin = http.get('https://<your-api-gateway-url>/login');
  check(resLogin, { 'status was 200': (r) => r.status == 200 });
  sleep(1);

  // Testar o endpoint /periodo-demonstrativo
  let resPeriodoDemonstrativo = http.get('https://<your-api-gateway-url>/periodo-demonstrativo');
  check(resPeriodoDemonstrativo, { 'status was 200': (r) => r.status == 200 });
  sleep(1);

  // Testar o endpoint /demonstrativo-pgto
  let resDemonstrativoPgto = http.get('https://<your-api-gateway-url>/demonstrativo-pgto');
